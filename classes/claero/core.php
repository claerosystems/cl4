...
class Claero_Core extends Kohana_Core {
    /**
	...
	/**
	* Sets the exception handler to the customized Claero version
	...
	*/
    public static function set_error_handlers() {
		...
		set_exception_handler(array('Claero', 'exception_handler'));
	...
	*/
	...
	public static function printr() {
		...
		if (Claero::is_dev()) {
			...
			if ( ! isset(Claero::$is_firephp)) {
				// See if it's available
				Claero::$is_firephp = in_array('firephp', array_keys(Kohana::modules()));
			}

			if (Claero::$is_firephp) {
				Fire::log($content);
			} else {
	...
	*/
	public static function translate_array($array, $key = FALSE) {
		foreach ($array as $key => $value) {
			if ($key) {
				if (is_array($value)) {
					$array[__($key)] = Claero::translate_array($value, $key);
				} else {
					$array[__($key)] = __($value);
				}
			} else {
				if (is_array($value)) {
					$array[$key] = Claero::translate_array($value, $key);
				} else {
					$array[$key] = __($value);
				}
			} // if
	...
	/**
	* Used in Form::phone(), ORM_Phone and Claero::format_phone() to break apart the phone number stored in the database as a string
	...
	* If a string is passed it will be parsed with Claero::parse_phone_value() first
	*
	* @param mixed $phone
	* @return string
	*/
	public static function format_phone($phone) {
		if ( ! is_array($phone)) {
			...
			$phone = Claero::parse_phone_value($phone);
		}

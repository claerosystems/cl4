...
class Claero_File {
	...
	* @var array
	*/
	private $options = array(); // defaults in config/claerofile.options
	...
	*
	* @param  array   $options	array of options to change the defaults in config/claerofile.options
	*/
	...
	/**
	* reset the options for the object with the passed $options array and fill in missing values from defaults in config/claerofile.options
	*
	* @param mixed $options (see config/claerofile.options array for values)
	*/
	public function set_options($options) {
		// get the default options from the config file
		$default_options = Kohana::config('claerofile.options');
	...
	public function upload($files_array_loc, $destination_folder, array $options = array()) {
		...
		if ($this->options['make_dir'] && ! file_exists($destination_folder)) {
			...
			if ( ! mkdir($destination_folder, 0755, TRUE)) {
				throw new Kohana_Exception('Could not create the destination path: :dest_folder: :exception_text:', array(':dest_folder:' => $destination_folder, ':exception_text:' => Kohana::exception_text($e)), Claero_Exception_File::DESTINATION_FOLDER_DOESNT_EXIST);
			}

		// make sure the destination folder exists
		} else if ( ! is_dir($destination_folder)) {
			throw new Claero_Exception_File('The destination folder for the uploaded file doesn\'t exist: :dest_folder:', array(':dest_folder:' => $destination_folder), Claero_Exception_File::DESTINATION_FOLDER_DOESNT_EXIST);
		} // if

        // no files received
        if ( ! isset($_FILES)) {
			throw new Claero_Exception_File('The $_FILES array is not set, therefore no files were saved', NULL, Claero_Exception_File::NO_FILES_RECEIVED);
        }
		...
		if (is_array($files_array_loc)) {
			...
			$name = ClaeroFile::get_files_array_value($files_array_loc, 'name');
			...
			if ( ! empty($name)){
				...
				$file_data = array(
					'name' => $name,
					'type' => ClaeroFile::get_files_array_value($files_array_loc, 'type'),
					'tmp_name' => ClaeroFile::get_files_array_value($files_array_loc, 'tmp_name'),
					'error' => ClaeroFile::get_files_array_value($files_array_loc, 'error'),
					'size' => ClaeroFile::get_files_array_value($files_array_loc, 'size'),
				);
			} else {
				throw new Claero_Exception_File('The field :name: in the $_FILES array was not set, so no file was processed', array(':name:' => implode('.', $files_array_loc)), Claero_Exception_File::FILE_NOT_SET);
			}

		} else if ( ! empty($_FILES[$files_array_loc]['name'])) {
			$file_data = $_FILES[$files_array_loc];
		} else {
			throw new Claero_Exception_File('The field :name: in the $_FILES array was not set, so no file was processed', array(':name:' => $files_array_loc), Claero_Exception_File::FILE_NOT_SET);
		}
		...
		if ($file_data['error'] > 0) {
			...
			throw new Claero_Exception_File('PHP File Upload Error (:error:) ' . $msg, array(':error:' => $file_data['error']), Claero_Exception_File::PHP_FILE_UPLOAD_ERROR);
		} // if
		...
		if ( ! is_uploaded_file($file_info['tmp_file'])) {
			throw new Claero_Exception_File('File received is not an uploaded file: :file:', array(':file:' => $file_info['tmp_file']), Claero_Exception_File::NOT_UPLOADED_FILE);
		...
		if ( ! $this->options['allow_any_file_type']) {
			...
			if ($this->options['ext_check_only']) {
				if ( ! in_array(strtolower($file_info['ext']), $this->options['allowed_extensions'])) {
					throw new Claero_Exception_File('The file extension ":ext:" is not an allowed extension', array(':ext:' => $file_info['ext']), Claero_Exception_File::EXTENSION_NOT_ALLOWED);
				}

			} else {
				if ( ! in_array($file_info['mime_type'], $this->options['allowed_types'])) {
					throw new Claero_Exception_File('The mime type ":mime:" is not an allowed mime type', array(':mime:' => $file_info['mime_type']), Claero_Exception_File::MIME_NOT_ALLOWED);
				}
			}
		} // if
		...
		$file_info['dest_file'] = ClaeroFile::get_destination_filename($file_info, $this->options['name_change_method'], $destination_filename_options);
		...
		if (file_exists($file_info['dest_file_path'])) {
			if ( ! $this->options['overwrite']) {
				throw new Claero_Exception_File('The destination file already exists :dest_file: for user file :user_file:', array(':dest_file:' => $file_info['dest_file'], ':user_file:' => $file_info['user_file']), Claero_Exception_File::DESTINATION_FILE_EXISTS);

			// we can overwrite, so remove the file first
			} else if ( ! $this->delete($file_info['dest_file_path'])) {
				throw new Claero_Exception_File('The destination file already exists :dest_file: but could not be deleted/overwritten', array(':dest_file:' => $file_info['dest_file']), Claero_Exception_File::DELETE_FILE_FAILED);
			}
		}
		...
		try {
			...
			throw new Claero_Exception_File('The uploaded file :user_file: could not be moved to :dest_file: with move_uploaded_file(), the error was: `' . $e->getMessage() . '`', array(
				':user_file:' => $file_info['user_file'],
				':dest_file:' => $file_info['dest_file']), Claero_Exception_File::MOVE_UPLOADED_FILE_FAILED);
		}
	...
	* This can be run from the object or statically which will then merge the default options in config/claerofile.options with the passed options
	*
	*/
	private static function get_mime_type_error_msg($html = TRUE, array $options = NULL) {
		if (isset($this) && $options === NULL) {
			...
			$options += Kohana::config('claerofile.options');
		}
	...
	*/
	public static function get_destination_filename(array $file_info, $name_change_method = 'keep', array $options = array()) {
		...
		if (in_array($name_change_method, $dest_required) && empty($options['name_change_text'])) {
			throw new Claero_Exception_File('Input Error: No destination filename received, defaulting to keep the original: "' . $file_info['orig_file'] . '"');
			$options['name_change_text'] = $file_info['orig_file'];
		}

		// overwrite/append/prepend/etc src filename
		// if these are added to, also need to edit with ClaeroEdit
		switch ($name_change_method) {
			...
			case 'random' :
				...
				$dest_file = time() . '_' . ClaeroFile::clean_filename(uniqid()) . '.' . $file_info['ext'];
				break;
		...
		if ($options['clean_filename']) {
			$dest_file = ClaeroFile::clean_filename($dest_file);
		}
	...
	public static function file_security_check($filename, $path_to_check_with = NULL) {
		if ($path_to_check_with === NULL) {
			if (defined('ABS_ROOT')) {
				...
				throw new Claero_Exception_File('No path to check with and the constant ABS_ROOT is not defined so no file security check can be performed');
			}
		}
	...
	*/
	public function copy_file_to_id($id, $original_filename, $move = FALSE) {
		if ( ! file_exists($original_filename)) {
			throw new Claero_Exception_File('The file to be copied to the new ID based filename does not exist: :file', array('file' => $original_filename), Claero_Exception_File::FILE_DOES_NOT_EXIST);

		} else if ( ! is_file($original_filename)) {
			throw new Claero_Exception_File('The file to be copied to the new ID based filename is not a file (possibly a directory): :file', array('file' => $filename), Claero_Exception_File::IS_NOT_REGULAR_FILE);
		}
		...
		} catch (Claero_Exception_File $e) {
			throw new Claero_Exception_File('Failed to copy file to id based filename: :msg', array('msg' => $e->getMessage()), Claero_Exception_File::ID_COPY_FAILED);
		}
	...
	*/
	public static function move($original_file, $destination_file, $overwrite = FALSE) {
		...
		if ( ! file_exists($original_file)) {
			throw new Claero_Exception_File('The file being moved does not exist: :file', array('file' => $original_file), Claero_Exception_File::FILE_DOES_NOT_EXIST);

		} else if (file_exists($destination_file)) {
			throw new Claero_Exception_File('The destination file exists: :file', array('file' => $destination_file), Claero_Exception_File::DESTINATION_FILE_EXISTS);

		} else if ( ! rename($original_file, $destination_file)) {
			throw new Claero_Exception_File('The file could not be moved. Original file: :orig_file Destination file: :dest_file', array('orig_file' => $original_file, 'dest_file' => $destination_file), Claero_Exception_File::MOVE_FILE_FAILED);
		} // if
	...
	*/
	public function copy($original_file, $destination_file) {
		// ensure the file we are working with exists
		if ( ! file_exists($original_file)) {
			throw new Claero_Exception_File('The file being copied does not exist: :file', array('file' => $original_file), Claero_Exception_File::FILE_DOES_NOT_EXIST);

		} else if ( ! copy($original_file, $destination_file)) {
			throw new Claero_Exception_File('The file could not be copied. Original file: :orig_file Destination file: :dest_file', array('orig_file' => $original_file, 'dest_file' => $destination_file), Claero_Exception_File::COPY_FILE_FAILED);
		} // if
	...
	*/
	public static function delete($filename) {
		if ( ! file_exists($filename)) {
			throw new Claero_Exception_File('The file you are trying to delete does not exist: :file', array('file' => $filename), Claero_Exception_File::FILE_DOES_NOT_EXIST);

		} else if ( ! unlink($filename)) {
			throw new Claero_Exception_File('Unable to delete/unlink the file: :file', array('file' => $filename), Claero_Exception_File::DELETE_FILE_FAILED);
		}
	...
	*/
	public static function stream($file_path, $display_filename = NULL, $disposition = 'attachment') {
		...
		if ( ! file_exists($file_path)) {
			throw new Claero_Exception_File('The file to be streamed does not exist: :file', array(':file' => $filepath), Claero_Exception_File::FILE_DOES_NOT_EXIST);

		} else if ( ! is_file($file_path)) {
			throw new Claero_Exception_File('The file to be streamed is not a file (possibly a directory): :file', array(':file' => $filepath), Claero_Exception_File::IS_NOT_REGULAR_FILE);
			...
			if ( ! headers_sent()) {
				...
				throw new Claero_Exception_File('The headers have already been sent so the headers to download the file can\'t be sent', array(), Claero_Exception_File::DOWNLOAD_HEADERS_CANT_BE_SENT);
			} // if headers sent
		} // if exists
	} // function

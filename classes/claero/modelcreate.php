...
class Claero_ModelCreate {
	...
	*
	*     echo claeroform::create_model('user',array()); <- todo: is this correct?
	*
	...
	*/
	public static function create_model($table_name, $options = array()) {
		...
		$model_code .= EOL;
		$model_code .= "/**" . EOL;
		$model_code .= " * This model was created using Claero_ORM and should provide " . EOL;
		...
		foreach ($columns as $column_name => $column_data) {
			...
			$model_code .= TAB . TAB . '\'' . $column_name . '\' => \'' . Claero::underscores_to_words($column_name_for_label) . '\',' . EOL;
		} // foreach
		$model_code .= TAB . ');' . EOL;
		...
		$model_code .= TAB . TAB . '* see the modules/claero/config/claeroorm.php for a full list of cl4-specific options and documentation on what the options do' . EOL;
		$model_code .= TAB . TAB . '*/' . EOL;
		...
		$display_order = 10;
		foreach ($columns as $column_name => $column_data) {
			$meta_data = Kohana::config('claeroorm.default_meta_data');

...
class Claero_ORM_File extends ORM_FieldType {
	public static function edit($column_name, $html_name, $value, array $attributes = NULL, array $options = array(), ORM $orm_model = NULL) {
		...
		return View::factory('claero/orm_file', $view_options);
	} // function

	public static function save($post, $column_name, array $options = array(), ORM $orm_model = NULL) {
		$file_options = $options['file_options'];

		$destination_folder = ClaeroFile::get_file_path($file_options['destination_folder'], $orm_model->table_name(), $column_name, $file_options);
		...
		if (Arr::get($post, $remove_checkbox_name, FALSE)) {
			try {
				if ($file_options['delete_files']) {
					...
					if (file_exists($file_to_delete) && ! is_dir($file_to_delete) && ! ClaeroFile::delete($file_to_delete)) {
		...
		// check if we are dealing with a post array
			...
			try {
				...
				$post_file_name = ClaeroFile::get_files_array_value($path_to_file, 'name');

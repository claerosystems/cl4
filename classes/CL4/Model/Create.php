...
class CL4_Model_Create {
	...
	*/
	...
	protected function build_meta_data($column_name, $column_data) {
		...
		if ($column_data['data_type'] == 'decimal' || $column_data['data_type'] == 'decimal unsigned') {
			// if the field is decimal(10,2) then the size should be 9 = 10 - 2 (decimals) + 1 (decimal point)
			$meta_data['field_attributes']['size'] = $meta_data['field_attributes']['maxlength'] = $column_data['numeric_precision'] - $column_data['numeric_scale'] + 1;
